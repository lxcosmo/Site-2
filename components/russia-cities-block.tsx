...
export default function RussiaCitiesBlock() {
  const cities = [
    { name: "Москва", icon: "✈" },
    { name: "Санкт-Петербург", icon: "✈" },
    { name: "Екатеринбург", icon: "✈" },
    { name: "Новосибирск", icon: "✈" },
    { name: "Красноярск", icon: "✈" },
    { name: "Владивосток", icon: "✈" },
    { name: "Казань", icon: "✈" },
    { name: "Краснодар", icon: "✈" },
  ]
        ...
        <div className="grid grid-cols-2 gap-6 max-w-4xl mx-auto">
          {cities.map((city, index) => (
            ...
            >
              ...
              <span className="text-lg font-semibold text-gray-800">{city.name}</span>
            </div>
          ))}
        </div>
      </div>
